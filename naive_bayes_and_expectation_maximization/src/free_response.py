...
def free_response():
    ...
    # FRQ 2a
    fx = np.exp(naive_bayes_em.p_x_y[:,1]) - np.exp(naive_bayes_em.p_x_y[:,0])
    fx_sorted = np.argsort(fx)
    dem = vocab[fx_sorted[0:5]]
    rep = vocab[fx_sorted[-6:-1]]
    print(f"Democrats: {dem}")
