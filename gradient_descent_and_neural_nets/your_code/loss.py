...
class SquaredLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        """
        N = y.shape[0]
        loss = sum([1/2 * (y[i] - X[i].dot(w))**2 for i in range(N)])/N
    ...
    def backward(self, X, w, y):
        ...
        """
        N = y.shape[0]
        gradient = 1/N * -(X.T).dot(y - X.dot(w))
...
class HingeLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        """
        N = y.shape[0]
        loss = 1/N * sum([max(0, 1 - y[i] * X[i,:].dot(w.T)) for i in range(N)])
    ...
    def backward(self, X, w, y):
        ...
        gradient = 0
        for i in range(N):
            gradient += (-y[i]*X[i,:])*(1 - y[i]*np.dot(w.T, X[i,:]) > 0)/N
